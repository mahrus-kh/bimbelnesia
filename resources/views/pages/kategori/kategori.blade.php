...
@section('title')
    <title>Kategori {{ request()->segment(2) }} | Bimbingan Belajar Indonesia - BimbelNesia</title>
@endsection
        ...
        <div class="row">
            <div class="col-md-3 mb-3">
                <div class="list-group">
                    <a href="{{ route('kategori.index') }}" class="list-group-item list-group-item-action font-weight-bold">Semua Kategori <span class="badge badge-light badge-pill font-weight-bold pull-right"></span></a>
                    @foreach($list_category as $row)
                        <a href="{{ route('kategori', $row->slug) }}" class="{{ request()->segment(2) === $row->slug ? 'active' : '' }} list-group-item list-group-item-action font-weight-bold">{{ $row->category }} <span class="badge badge-light badge-pill font-weight-bold pull-right">{{ $row->count }}</span></a>
                    @endforeach
                </div>
            </div>
            <div class="col-md-9">
                ...
                <div class="mt-2 text-center">
                    {{ $lembaga->withPath(request()->segment(2))->links('templates.partials.pagination') }}
                </div>
            </div>
        </div>
    </div>
